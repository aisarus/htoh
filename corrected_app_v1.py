...
# Инициализируем клиент OpenAI v1+
...
st.markdown("**Введите сообщения от двух людей, и модель интерпретирует их через EFM (Emotion, Fact, Narrative, Meta-context, Bias).**")
...
3. Narrative (N): цель сообщения (например: просьба, защита, провокация, исповедь, утешение и т.д.)
4. Meta-context (M): краткое описание положения говорящего
5. Bias (B): на что влияет история сообщений и предыдущие взаимодействия
...
def analyze_efm(text):
    ...
    response = client.chat.completions.create(
        model="gpt-4",
        messages=[
            {"role": "system", "content": "Ты — интерпретатор по модели EFM. Отвечай строго по структуре и по делу."},
